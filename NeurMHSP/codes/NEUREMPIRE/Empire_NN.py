...
def run_empire(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
    ...
    def second_Obj_rule(model):
        return sum(model.discount_multiplier[i]*sum(model.shedcomponent[i] + model.operationalcost[i]) for i in model.PeriodActive)
    model.sub_Obj = Objective(rule=second_Obj_rule, sense=minimize)
    ...
    full_filename='training_set.csv'
    
    
    gen_capital_costs_df = pd.read_csv(tab_file_path + "/Generator_CapitalCosts.tab", sep="\t")
    transmission_costs_df = pd.read_csv(tab_file_path + "/Transmission_CapitalCosts.tab", sep="\t")
    stor_pw_costs_df = pd.read_csv(tab_file_path + "/StoragePower_CapitalCosts.tab", sep="\t")
    stor_en_costs_df = pd.read_csv(tab_file_path + "/StorageEnergy_CapitalCosts.tab", sep="\t")

    upper_limit_genInvCap = gen_capital_costs_df['Cost'].max()
    upper_limit_transmisionInvCap = transmission_costs_df['Cost'].max()
    upper_limit_storPWInvCap = stor_pw_costs_df['Cost'].max()
    upper_limit_storENInvCap = stor_en_costs_df['Cost'].max()

    GeneratorsOfNode = list(model.GeneratorsOfNode.data())
    BidirectionalArc = list(model.BidirectionalArc.data())
    StoragesOfNode = list(model.StoragesOfNode.data())
    PeriodActive = list(model.PeriodActive.data())


    for i in range(number_sample):
        first_stage_decision_set = {
        'genInvCap_values': {},
        'transmisionInvCap_values': {},
        'storPWInvCap_values': {},
        'storENInvCap_values': {}
        }

        # 각 변수에 대한 랜덤 값 생성 및 딕셔너리에 저장
        for n, g in GeneratorsOfNode:
            for i in PeriodActive:
                genInvCap_values[(n, g, i)] = random.uniform(0, upper_limit_genInvCap)

        for n1, n2 in BidirectionalArc:
            for i in PeriodActive:
                transmisionInvCap_values[(n1, n2, i)] = random.uniform(0, upper_limit_transmisionInvCap)

        for n, b in StoragesOfNode:
            for i in PeriodActive:
                storPWInvCap_values[(n, b, i)] = random.uniform(0, upper_limit_storPWInvCap)

        for n, b in StoragesOfNode:
            for i in PeriodActive:
                storENInvCap_values[(n, b, i)] = random.uniform(0, upper_limit_storENInvCap)
    
        expected_second_stage_value = Q(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
                solver, temp_dir, FirstHoursOfRegSeason, FirstHoursOfPeakSeason, lengthRegSeason,
                lengthPeakSeason, Period, Operationalhour, Scenario, Season, HoursOfSeason,
                discountrate, WACC, LeapYearsInvestment, IAMC_PRINT, WRITE_LP,
                PICKLE_INSTANCE, EMISSION_CAP, USE_TEMP_DIR, LOADCHANGEMODULE,
        ...
        first_stage_decisions_lst.append(first_stage_decision_set)
