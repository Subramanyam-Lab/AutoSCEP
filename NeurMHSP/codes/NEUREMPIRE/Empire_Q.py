...
def Q(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
    ...
    def lifetime_rule_gen(model, n, g, i):
        ...
        return sum(genInvCap[n,g,j]  for j in model.PeriodActive if j>=startPeriod and j<=i )- model.genInstalledCap[n,g,i] + model.genInitCap[n,g,i]== 0   #
    ...
    def lifetime_rule_storEN(model, n, b, i):
        ...
        return sum(storENInvCap[n,b,j]  for j in model.PeriodActive if j>=startPeriod and j<=i )- model.storENInstalledCap[n,b,i] + model.storENInitCap[n,b,i]== 0   #
    ...
    def lifetime_rule_storPOW(model, n, b, i):
        ...
        if value(1+i-model.storageLifetime[b]*(1/model.LeapYearsInvestment))>startPeriod:
            startPeriod=value(1+i-model.storageLifetime[b]/model.LeapYearsInvestment)
        return sum(storPWInvCap[n,b,j]  for j in model.PeriodActive if j>=startPeriod and j<=i )- model.storPWInstalledCap[n,b,i] + model.storPWInitCap[n,b,i]== 0   #
    ...
    def lifetime_rule_trans(model, n1, n2, i):
        ...
        return sum(transmisionInvCap[n1,n2,j]  for j in model.PeriodActive if j>=startPeriod and j<=i )- model.transmissionInstalledCap[n1,n2,i] + model.transmissionInitCap[n1,n2,i] == 0   #
    ...
    def investment_gen_cap_rule(model, t, n, i):
        return sum(genInvCap[n,g,i] for g in model.Generator if (n,g) in model.GeneratorsOfNode and (t,g) in model.GeneratorsOfTechnology) - model.genMaxBuiltCap[n,t,i] <= 0
    ...
    def investment_trans_cap_rule(model, n1, n2, i):
        return transmisionInvCap[n1,n2,i] - model.transmissionMaxBuiltCap[n1,n2,i] <= 0
    ...
    def investment_storage_power_cap_rule(model, n, b, i):
        return storPWInvCap[n,b,i] - model.storPWMaxBuiltCap[n,b,i] <= 0
    ...
    def investment_storage_energy_cap_rule(model, n, b, i):
        return storENInvCap[n,b,i] - model.storENMaxBuiltCap[n,b,i] <= 0
    ...
    # If the solution is optimal, retrieve the objective value
        second_obj_value = value(model.sub_Obj)
        print("second_obj_value: ",second_obj_value)
