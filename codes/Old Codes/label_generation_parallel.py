...
def label_generation(file_num, specific_period, lengthRegSeason, FSD, num_workers):
    ...
    accumulated_feature_vectors = []
    results, first_obj, v_dict, initial_features = batch_estimator(file_num, specific_period, initial_num_sce, lengthRegSeason, FSD, initial_seeds, num_workers)
    accumulated_feature_vectors.extend(initial_features)
    ...
    while True:
        ...
        if not h_is_fixed and rel_half > threshold_h:
            ...
            results, first_obj, v_dict, new_features = batch_estimator(file_num, specific_period, N, lengthRegSeason, FSD, used_seeds, num_workers)
            accumulated_feature_vectors = new_features
            continue
        ...
        if r_error > threshold_r:
            ...
            more_results, _, _ , more_features= batch_estimator(file_num, specific_period, needed, lengthRegSeason, FSD, more_seeds, num_workers)
            results = np.concatenate([results, more_results])
            accumulated_feature_vectors.extend(more_features)
            continue
    ...
    # 최종적으로 누적된 특징 벡터들의 평균 계산
    avg_feature_vector = []
    if accumulated_feature_vectors:
        valid_features = [f for f in accumulated_feature_vectors if f is not None and len(f) > 0]
        if valid_features:
...
def main():
    ...
    try:
        ...
        output_dir = f"training_data_distributed/file_{file_num}"
