...
def generate_kip_instance(n, k_ratio):
    ...
    while any(weight > b for weight in weights):
        weights = np.random.randint(1, 101, size=n)
    instance = {
        "n": n,
        "k": k,
...
def generate_data_for_instance(args):
    ...
    while count < num_samples:
        ...
        chosen = np.random.choice(n, subset_size, replace=False)
        x[chosen] = 1
        obj_val = solve_follower(x, instance)
        # f_i: 7 features
        # 1) (p_i/a_i)/max_ratio
        # 2) p_i
        # 3) a_i
        ...
        # 7) obj^{dg}/n
        f_i = np.stack([
            (profits/(weights+1e-9))/max_ratio,
            profits,
            weights,
            np.full(n, k/n),
            x_dg,
        ...
        # h(x_i): f_i plus x_i and y_i^{g}
        # total: 7 + 1 (x_i) + 1 (y_i^g) = 9 features
        h_x_i = np.concatenate([f_i, x.reshape(-1,1), y_g.reshape(-1,1)], axis=1)

        c_i = profits.copy()

        f_i_list.append(f_i)
        h_x_i_list.append(h_x_i)
        c_i_list.append(c_i)
        x_i_list.append(x)
        y_list.append(obj_val)
        count += 1

    return (np.array(f_i_list), np.array(h_x_i_list), np.array(c_i_list),
            np.array(x_i_list), np.array(y_list))
...
if __name__ == "__main__":
    ...
    # Generate instances for the current combination of n and k_ratio
    instances = [generate_kip_instance(n, k_ratio) for _ in range(instances_per_combination)]

    # Generate data in parallel
    args_list = [(inst, num_samples_per_instance) for inst in instances]
    num_workers = min(cpu_count(), len(instances))
    with Pool(num_workers) as pool:
        results = pool.map(generate_data_for_instance, args_list)

    # Accumulate data from all instances
    f_i_list, h_x_i_list, c_i_list, x_i_list, y_list = [], [], [], [], []
    for (f_i_arr, h_x_i_arr, c_i_arr, x_i_arr, y_arr) in results:
        f_i_list.append(f_i_arr)
        h_x_i_list.append(h_x_i_arr)
        c_i_list.append(c_i_arr)
        x_i_list.append(x_i_arr)
        y_list.append(y_arr)
