...
def main(solution_numbers,numsce,seednum,model,scenario_set_num):
    ...
    # fsd_file_path = f"100_seed_5_inv_cap.csv"
    # fsd_file_path = f"sol_sets/ef_solution_{solution_numbers}_{numsce}.csv"
    ...
    first_stage_obj, second_obj_val, ratio, total_count, exceed_count = run_second_stage(
        tab_file_path=scenario_folder,
        temp_dir=temp_dir,
    ...
    result = first_stage_obj + second_obj_val
    logging.info(f"objective_value_ML: {result}")
    logging.info(f"first_stage_value_ML: {first_stage_obj}" )
    logging.info(f"expected_second_stage_value_ML: {second_obj_val}")
