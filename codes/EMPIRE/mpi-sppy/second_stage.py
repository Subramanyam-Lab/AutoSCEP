...
def run_second_stage(tab_file_path, temp_dir, FirstHoursOfRegSeason, FirstHoursOfPeakSeason, lengthRegSeason,
               ...
               LOADCHANGEMODULE,seed,file_num,north_sea,version):
    ...
    if tab_file_path is None:
        scenariopath = f"Data handler/scenarios/{file_num}"
    else:
...
#        f.close()
    ...
    ratio, total_count, exceed_count = proportional_lost_load(instance)
    # Q_per_scenario, mean_q, std_q = compute_second_stage_stats(instance)
    # return Q_per_scenario, mean_q, std_q

    return first_stage_value, obj_value, ratio, total_count, exceed_count
