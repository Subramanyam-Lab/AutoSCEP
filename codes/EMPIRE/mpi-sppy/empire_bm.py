...
def get_memory_usage_mb():
    ...
    # resident set size (RSS) is a good measure of non-swapped physical memory
...
def Progressive_Hedging(options, scenario_names, seed, num_sce, time_limit):
    ...
    if ph.cylinder_rank == 0:
        ...
        log_file = "full_runtime_log_ph.csv"
...
if __name__ == '__main__':
    ...
    if method == "PH":
        ...
        #     "sp_solver_options" : {"threads": 4, "MIPGap":1e-2, "OutputFlag": 1},
        #     "root_solver_options": {"threads": 4, "MIPGap":1e-2,"OutputFlag": 1},
        #     "store_subproblems": True,
        #     "valid_eta_lb": {name: 1e+11 for name in scenario_names},
        #     "max_iter": 1000,
        #     "rel_gap": 0.01,           
        #     "abs_gap": 1e+10,         
        ...
        options_BD = {
            ...
            "sp_solver_options": {
                ...
                "MIPGap": 1e-2,
                # "NumericFocus": 3,
                # "BarHomogeneous": 1# barrier 수치 안정화
                # "Method": 2, "Crossover": 0,  # LP/Barrier만 쓰고 싶으면 고려
            },
            "root_solver_options": {
                "Threads": 4,
                "MIPGap": 1e-2,
            ...
            "store_subproblems": True,
            "valid_eta_lb": {name: 0.0 for name in scenario_names},  # 또는 아예 제거
            "max_iter": 1000,
            "rel_gap": 0.01,
            "abs_gap": 0.0,
            "max_stalled_iters": 20,
    ...
    # Gather all individual memory usages onto the rank 0 process
    all_mems = comm.gather(final_memory_mb, root=0)
    rank = comm.Get_rank()
    if rank == 0:
        total_mem = sum(all_mems)
        print("\n--- Memory Usage Report ---")
        print(f"Number of MPI processes: {len(all_mems)}")
        print(f"Peak memory per process (MB): {[f'{mem:.2f}' for mem in all_mems]}")
        print(f"**Total Estimated Peak Memory Usage: {total_mem:.2f} MB ({total_mem/1024:.2f} GB)**")
        print("---------------------------\n")