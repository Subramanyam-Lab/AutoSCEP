...
def run_embedded_empire(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
    ...
    model.nodeLostLoadCost = Param(model.Node, model.Period, default=10)
    ...
    pca_v       = joblib.load('scaler_pca/pca.pkl')
    scaler_v    = joblib.load('scaler_pca/scaler.joblib')
    linear_reg  = joblib.load('scaler_pca/linear_regression_model.joblib')
    scaler_y    = joblib.load('scaler_pca/scaler_y_for_tree_and_lin.joblib')

    mean_v_input    = scaler_v.mean_
    scale_v_input   = scaler_v.scale_
    pca_components  = pca_v.components_       # shape=(n_components, input_dim)
    reg_coef        = linear_reg.coef_        # shape=(n_components,)
    reg_intercept   = linear_reg.intercept_   # float
    out_mean        = scaler_y.mean_
    out_scale       = scaler_y.scale_

    v_index_list = []

    def custom_build_rule(model):    
        for p in model.PeriodActive:
            for (n,g) in model.GeneratorsOfNode:
                v_index_list.append(('gen', n, g, p))
            for (n1,n2) in model.BidirectionalArc:
                v_index_list.append(('trans', n1, n2, p))
            for (n,b) in model.StoragesOfNode:
                v_index_list.append(('storPW', n, b, p))
            for (n,b) in model.StoragesOfNode:
                v_index_list.append(('storEN', n, b, p))
    model.build_custom = BuildAction(rule=custom_build_rule)

    model.V_IDX = RangeSet(0, len(v_index_list)-1)

    model.scaledV = Var(model.V_IDX, domain=Reals)  
    n_components  = pca_components.shape[0]
    model.pcaV    = Var(range(n_components), domain=Reals)
    model.y_approx         = Var(domain=Reals) 
    model.y_approx_scaled  = Var(domain=Reals)  

    def scaled_v_def_rule(m, i):
        vtype, *others = v_index_list[i]
        if vtype == 'gen':
            (n,g,p) = others
            original_v = m.genInstalledCap[n,g,p]
        elif vtype == 'trans':
            (n1,n2,p) = others
            original_v = m.transmissionInstalledCap[n1,n2,p]
        elif vtype == 'storPW':
            (n,b,p) = others
            original_v = m.storPWInstalledCap[n,b,p]
        elif vtype == 'storEN':
            (n,b,p) = others
            original_v = m.storENInstalledCap[n,b,p]
        else:
            return Constraint.Skip

        return m.scaledV[i] == (original_v - mean_v_input[i]) / scale_v_input[i]
    model.scaled_v_def = Constraint(model.V_IDX, rule=scaled_v_def_rule)

    def pca_v_def_rule(m, j):
        return m.pcaV[j] == sum(pca_components[j, i]*m.scaledV[i] for i in m.V_IDX)
    model.pca_v_def = Constraint(range(n_components), rule=pca_v_def_rule)

    def y_approx_def_rule(m):
        return m.y_approx == reg_intercept + sum(reg_coef[j]*m.pcaV[j] for j in range(n_components))
    model.y_approx_def = Constraint(rule=y_approx_def_rule)

    def y_approx_scaled_rule(m):
        return m.y_approx_scaled == (m.y_approx * out_scale + out_mean)
    model.y_approx_scaled_def = Constraint(rule=y_approx_scaled_rule)
    ...
    def Obj_rule(model):
        ...
        second_stage_value = model.y_approx_scaled
        return first_stage_value + second_stage_value
...
def get_results(instance):
    ...
    output_file_path = os.path.join(output_dir, f"ml_embedded_empire_inv_cap.csv")
...
if __name__ == "__main__":
    ...
    objective_value = run_embedded_empire(name = name, 
            tab_file_path = tab_file_path,
