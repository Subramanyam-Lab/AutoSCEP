...
def embed_empire_embedding(version, tab_file_path, result_file_path, temp_dir, FirstHoursOfRegSeason, FirstHoursOfPeakSeason, lengthRegSeason,
    ...
    n_features = 616
    scaler_v = joblib.load(f'ML_models_adaptive/{file_prefix}_seed{Seed}_v_scaler.gz')
    ...
    scaler_y = joblib.load(f'ML_models_adaptive/{file_prefix}_seed{Seed}_y_scaler.gz')
    onnx_path = f'ML_models_adaptive/{file_prefix}_seed{Seed}_nn_regressor.onnx'
    ...
    if model_type == "MLP":
        ...
        embedded_instance = integrate_linear_regression_with_pyomo_model(f'ML_models_adaptive/{file_prefix}_seed{Seed}_lr.joblib', base_instance, output_var_name, scaler_y) # LR
