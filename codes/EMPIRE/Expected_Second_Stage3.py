...
def run_second_stage(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
               ...
               PICKLE_INSTANCE, EMISSION_CAP, USE_TEMP_DIR, LOADCHANGEMODULE,seed,specific_period,file_num,north_sea,hour_decision,version):
    ...
    if hour_decision:
        ...
        scenariopath = f"Data handler/scenarios/file_num_{file_num}/{seed}"
...
#        f.close()
    ...
    proportional_lost_load(instance)
    
    
    # return first_stage_value, obj_value, v_dict 
    Q_per_scenario, mean_q, std_q = compute_second_stage_stats(instance)
                            ...
                            for b in instance.Storage:
                                if (n, b) in instance.StoragesOfNode:
                                    if h in instance.FirstHoursOfRegSeason or h in instance.FirstHoursOfPeakSeason:
                                        instance.storOperational[n,b,h,i,w] = instance.storOperationalInit[b]*instance.storENInstalledCap[n,b,i] + instance.storageChargeEff[b]*instance.storCharge[n,b,h,i,w]-instance.storDischarge[n,b,h,i,w] 
                                        stor_prev_level += value(instance.storOperational[n, b, h, i, w])
                                    else:
...
def calculate_Q(instance):
    ...
    for w in instance.Scenario: 
        for i in instance.PeriodActive:
            ...
            operational_cost = sum(value(instance.operationalDiscountrate)*value(instance.seasScale[s]) * value(instance.genMargCost[g,i]) * value(instance.genOperational[n,g,h,i,w])
                                for (n,g) in instance.GeneratorsOfNode 
                                for (s,h) in instance.HoursOfSeason) 
            
            shed_cost = sum(value(instance.operationalDiscountrate)*value(instance.seasScale[s]) * value(instance.nodeLostLoadCost[n,i]) * value(instance.loadShed[n,h,i,w])
                            for n in instance.Node 
                            for (s,h) in instance.HoursOfSeason)
...
def lost_load_amt(instance):
    # Calculate the total operational cost over all periods
    total_ll_amt = 0
    for i in instance.PeriodActive:
        shed_amt = sum(
            value(instance.operationalDiscountrate)*value(instance.seasScale[s])* value(instance.sceProbab[w]) * value(instance.loadShed[n, h, i, w])
            for n in instance.Node
            for w in instance.Scenario
            for (s, h) in instance.HoursOfSeason
        )
...
def proportional_lost_load(instance):
    ...
    print(f"lost load proportional: {ratio}, {total_count}, {exceed_count}")
...
def compute_second_stage_stats(instance):
    Q_per_scenario = {}
    # 각 시나리오별로 Q_i(operational + shed)를 합산
    for w in instance.Scenario:
        q_vals = []
        for i in instance.PeriodActive:
            # operational cost
