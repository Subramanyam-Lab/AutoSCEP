...
def run_empire(name, tab_file_path, result_file_path, scenariogeneration, scenario_data_path,
    ...
    results = opt.solve(instance, tee=True, logfile=result_file_path + '/logfile_' + name + '.log')

    # 솔루션 후 결과 처리
    num_scenarios = len(instance.Scenario)
    
    # 병렬로 기대값 계산 (시나리오 및 시즌 단위)
    expected_second_stage_value = compute_expected_second_stage_value_parallel(instance, num_scenarios, seed)
